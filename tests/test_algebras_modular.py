...
from src.algebras.axiom import AssociativityAxiom, CommutativityAxiom
...
class TestAlgebrasModular(unittest.TestCase):
    def test_z2_abelian_group(self):
        ...
        self.assertEqual(g.operation(1, 1), 0)
    ...
    def test_z3_abelian_group(self):
        ...
        self.assertEqual(g.operation(1, 2), 0)
    ...
    def test_monoid_not_group(self):
        ...
        with self.assertRaisesRegex(ValueError, "Axiom Inverse Existence is not satisfied"):
    ...
    def test_semigroup_not_monoid(self):
        ...
        validator = FiniteAxiomValidator(s.operation.table)
        self.assertTrue(validator.validate(s, AssociativityAxiom()))
        with self.assertRaisesRegex(ValueError, "Axiom Identity Existence is not satisfied"):
    ...
    def test_magma_not_semigroup(self):
        ...
        with self.assertRaisesRegex(ValueError, "Axiom Associativity is not satisfied"):
    ...
    def test_field_z6_validation(self):
        ...
        add_op = lambda a, b: (a + b) % 6
        mul_op = lambda a, b: (a * b) % 6
        with self.assertRaisesRegex(ValueError, "Axiom Multiplicative Inverses is not satisfied"):
    ...
    def test_commutative_ring_validation(self):
        ...
        mul_op = lambda a, b: (a * b) % 2
        r = Ring(elements, add_op, mul_op)
        validator = FiniteAxiomValidator(r.multiplication.table)
        self.assertTrue(validator.validate(r, CommutativityAxiom()))
    ...
    def test_distributivity_violation(self):
        ...
        add_op = lambda a, b: (a + b) % 2
        mul_op = lambda a, b: 1
        with self.assertRaisesRegex(ValueError, "Axiom Distributivity is not satisfied"):
            Ring(elements, add_op, mul_op)
