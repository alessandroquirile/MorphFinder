...
import { X, Grid, Code, Upload, Save } from 'lucide-react';
          ...
          <div className="max-w-2xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Structure Name</label>
                <input 
                  type="text" 
                  value={name} 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Elements (comma-separated)</label>
                <input 
                  type="text" 
                  value={elementsInput} 
            ...
            {activeTab === 'table' && (
              <div className="space-y-4">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Binary Operation Table (*)</label>
                <div className="inline-block border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                  <table className="border-collapse">
                    <thead>
                      <tr>
                        <th className="bg-slate-50 border-b border-r border-slate-200 p-3 text-slate-400 text-xs font-bold w-12">*</th>
                        {elements.map((el, i) => (
                          <th key={i} className="bg-slate-50 border-b border-r border-slate-200 p-3 text-indigo-600 text-xs font-bold w-12">{el}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
            ...
            {activeTab === 'dsl' && (
              <div className="space-y-4">
                <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
                  <h4 className="text-xs font-bold text-indigo-700 uppercase mb-2">Expression Guide</h4>
                  <p className="text-sm text-indigo-600 leading-relaxed">
                    Write a Python-style expression using variables <b>a</b>, <b>b</b>, and <b>n</b> (set size).
                    <br/>Example: <code className="bg-white px-1.5 py-0.5 rounded border border-indigo-200">(a + b) % n</code>
                  </p>
                </div>
                
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Operation Formula</label>
                <div className="relative group">
                  ...
                  />
                  <Code className="absolute right-4 top-4 text-slate-200 group-focus-within:text-indigo-200 transition-colors" size={24} />
                </div>
              </div>
            )}

            {activeTab === 'json' && (
              <div className="p-12 text-center border-2 border-dashed border-slate-200 rounded-3xl text-slate-400 bg-slate-50">
